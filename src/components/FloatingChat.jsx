...
const FloatingChat = () => {
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-[9999] bg-white shadow-2xl rounded-3xl border border-gray-100 flex flex-col transition-all duration-300 ${isMinimized ? 'w-72 h-16' : 'w-96 h-[500px]'}`}>
      {/* Header */}
      <header className="h-16 border-b border-gray-100 flex items-center justify-between px-4 bg-purple-600 rounded-t-3xl text-white">
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={closeFloatingChat} className="p-1.5 hover:bg-rose-500 rounded-lg transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
      </header>
