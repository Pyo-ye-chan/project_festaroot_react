  ...
  const handleBlacklist = async (member) => {
    if (window.confirm(`${member.nickname} 회원을 블랙리스트로 등록하시겠습니까?\n등록 시 영구적으로 서비스 이용이 제한됩니다.`)) {
      try {
        ...
        fetchMembers();
        fetchStats();
        if (isDetailModalOpen && detailData?.memberInfo?.id == member.id) {
          handleOpenDetail(member.id);
        }
      } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-50">
          ...
          {/* [UI 수정] 일괄 조작 제거 후 허전함을 채워줄 깔끔한 정보 배지 추가 */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1150px] table-fixed text-left">
            <colgroup>
              <col className="w-[60px]" /> {/* [UI 수정] w-[50px] -> w-[60px] 순번용 너비 조정 */}
              ...
              {isLoading && !isDetailModalOpen ? (
                ...
                displayedMembers.map((member, index) => (
                  <tr key={`${member.id}-${index}`} className="text-sm hover:bg-gray-50/50 transition">
                    {/* [UI 수정] 페이지네이션 공식이 결합된 연속 일련순번 출력 */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-5xl rounded-3xl bg-white shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 bg-gray-50/50">
              ...
              <button onClick={() => setIsDetailModalOpen(false)} className="p-2 rounded-xl bg-white border border-gray-100 text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition">
                <X size={18} />
              </button>
            </div>
              ...
              <div className="lg:col-span-4 space-y-5">
                <div className="rounded-2xl border border-gray-100 bg-gray-50/30 p-5 space-y-4">
                  ...
                  <div className="space-y-3 text-xs font-bold text-gray-500">
                    ...
                    <div className="flex justify-between"><span>가입 일시</span><span className="text-gray-700">{detailData.memberInfo.joinedAt}</span></div>
                    <div className="flex justify-between"><span>최근 로그인</span><span className="text-gray-700">{detailData.memberInfo.lastLogin || '-'}</span></div>
                    <div className="flex justify-between"><span>제재 해제 기한</span><span className="text-orange-600 font-black bg-orange-50 px-1.5 rounded">{detailData.memberInfo.suspensionEndDate || '-'}</span></div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-gray-100 p-5 space-y-3 bg-white">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button onClick={() => handleBlacklist(detailData.memberInfo)} className="h-11 rounded-xl bg-red-50 border border-red-100 text-red-500 font-black text-xs hover:bg-red-100 transition flex items-center justify-center gap-1"><Ban size={12} /> 블랙리스트</button>
                  </div>
                </div>
              </div>
                  ...
                  <table className="w-full min-w-[650px] text-left border-collapse table-fixed">
                    ...
                    <tbody className="divide-y divide-gray-50 text-xs font-semibold text-gray-600">
                      {detailData.reportHistory.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-16 text-center font-bold text-gray-400 bg-gray-50/10">
                            이 회원에 대해 인입된 게시글/댓글 신고 이력이 전혀 존재하지 않습니다.
                          </td>
                        </tr>
                      ) : (
                          ...
                          <tr key={item.historyId} className="hover:bg-gray-50/40 transition">
                            <td className="px-4 py-3.5">
                              <div className="flex items-center gap-1.5">
                                <span className="px-1.5 py-0.5 rounded bg-gray-100 text-gray-700 font-black text-[10px]">
                                  {REPORT_TYPE_LABELS[item.reportType] || item.reportType}
                                </span>
                                <span className="text-[10px] text-[#6d3df2] font-black font-mono" title="원본 신고 번호">
                                  #{item.reportId || item.historyId}
                                </span>
                              </div>
                              <p className="text-[10px] text-gray-400 mt-1 truncate">신고자: {item.reporterId || '비회원'}</p>
                            </td>
                            <td className="px-4 py-3.5 break-words whitespace-normal font-bold pr-2 text-gray-700" title={item.reason}>
                              {REASON_LABELS[item.reason] || item.reason}
                            </td>
                            <td className="px-4 py-3.5 text-center">
                              <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-black ${reportStatusClass[item.resultStatus]}`}>
                                {REPORT_STATUS_LABELS[item.resultStatus]}
                              </span>
                            </td>
                            <td className="px-4 py-3.5 font-medium text-gray-500 break-words whitespace-normal" title={item.adminMemo}>{item.adminMemo || '-'}</td>
                            <td className="px-4 py-3.5 text-center text-[11px] font-bold text-gray-400">{item.createdAt}</td>
                          </tr>
                        ))
                      )}
                    </tbody>
