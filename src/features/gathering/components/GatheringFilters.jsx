...
const GatheringFilters = ({ categories, activeTab, onTabChange, keyword, onKeywordChange }) => (
  <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex flex-col xl:flex-row gap-6 items-center">
    <div className="flex flex-row flex-nowrap sm:flex-wrap gap-1.5 sm:gap-2 flex-grow overflow-x-auto scrollbar-hide w-full sm:w-auto">
      {categories.map(cat => (
        <button
          key={cat}
          onClick={() => onTabChange(cat)}
          className={`px-2.5 py-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-2xl font-black text-[11px] sm:text-sm transition-all whitespace-nowrap shrink-0 flex-1 text-center sm:flex-none ${
            activeTab === cat 
